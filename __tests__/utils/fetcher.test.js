...
describe('fetcher', () => {
    ...
    test('check postJSON()', async () => {
        ...
        const headers = new Headers();
        headers.append('authorization', 'Basic Auth');
        await expect(fetcher.postJSON('http://some/test', {a: '123', b: 456}, headers)).resolves.toBe(true);
        await expect(fetcher.postJSON('http://some/test', {a: '123', b: 456})).rejects.toEqual(expect.any(Error));
